...
This brief gives the fastest current-state read for the automation agency without replacing the longer roadmap, launch plan, deployment handoff, pipeline files, launch evidence log, or tool access register.
...
On future runs, read this file first for the quick state, then inspect the specific control document for the task being executed. For website deployment or launch work, use `agency-ops/ops/website-launch-evidence-log.md` alongside the deployment handoff ticket and launch decision record template. For tool/app permission questions, use `agency-ops/ops/tool-access-and-approval-register.md`. If this brief conflicts with a more recently updated roadmap, launch-status record, lead record, deployment record, evidence log, approval register, or user instruction, use the newer evidence and update this brief.